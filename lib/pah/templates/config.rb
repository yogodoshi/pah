  ...
  module Templates
    class Config < Pah::Template
      ...
      def create_heroku_app?
        Pah.configuration.heroku[:create?] = will_you_like_to? 'create Heroku apps?'.red
      end
      ...
      def add_custom_domain?
        answer = ask 'Add custom domain (customdomain.com) or leave blank:'.red
        Pah.configuration.heroku[:domain] = answer
      end

      def add_collaborators?
        answer = ask "Add collaborators? Type the email's separated by comma (,):".red
